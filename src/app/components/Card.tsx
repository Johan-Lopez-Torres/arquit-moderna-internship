...
export const CardServiceLanding = () => {
  const dataService = DataService.map((service, index) => (
    <Link key={index} href={`${service.endpoint}   `}>
      <div className="card  bg-base-100 shadow-xl w-56 h-80 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110  hover:text-zinc-200 duration-200      border-teal-600 hover:border-4 rounded-none mb-6">
        <figure className=" ">
          <img
            src={service.image}
            alt="Movie"
